...
#' @param feature.matrix normalized gene by cell matrix 
#' @export
AddassGeneToTreePairwise <- function(d, count, feature.matrix=NULL, ntop=10, addAUC=TRUE, subsample=T,
                                     ncells=200, n.core=10){
  # TO DO: 
  # add adaptive weights to down-weight genes that appear in the upstream
  # add marker stability/interaction scores        
  ...
  calculate_marker <- function(d){
    for(i in seq_len(length(d))){
      ...
      if(!is.leaf(d[[i]])){
        ...
        groups <- setNames(c(rep("g1", length(attr(d[[i]][[1]], "nodesinfo"))),
                             rep("g2", length(attr(d[[i]][[2]], "nodesinfo")))), cells)
        ...
        if(addAUC){
          genesets <- c(diffgene.list[[1]]$Gene, diffgene.list[[2]]$Gene)
          model.res <- trainClassifier(genesets, groups, feature.matrix, model="rf", subsample=subsample, ncells=ncells,
                                       n.core=n.core)
          attr(d[[i]], "auc") <- round(model.res$perf.res$auc, 3)
  ...
  groups <- setNames(c(rep("g1", length(attr(d[[1]], "nodesinfo"))),
                       rep("g2", length(attr(d[[2]], "nodesinfo")))), cells)
  diffgene.list <- GetDifferentialGenes(count, groups=groups, ntop=ntop, upregulated.only=TRUE)
  if(addAUC){
    if(is.null(feature.matrix)){
      stop("Please provide feature matrix if addAUC is True")
    }
    
    genesets <- c(diffgene.list[[1]]$Gene, diffgene.list[[2]]$Gene)
    model.res <- trainClassifier(genesets, groups, feature.matrix, model="rf", subsample=subsample, ncells=ncells,
                                 n.core=n.core)
    attr(d[[1]], "auc") <- round(model.res$perf.res$auc, 3)
  }
...
#' Generate optimal structure based on node seperation power 
#' for each path from root to leaf, search the optimal node with maximum power to separate branches, branches are merged
#'   if it has seperation power smaller than defined cutoff
#' @param furthurStop go one branch further to avoid early stop 
#' @export
heuristicMerge2 <- function(d, cutoff=0.9, furthurStop=TRUE){
  is.good.split <- function(d, furthurStop=TRUE, cutoff=0.9){
    good.split <- FALSE
    if(is.leaf(d)){
      return(good.split)
    }
    ...
    if(is.null(auc.left.child)){
      auc.left.child <- 0
    }
    if(is.null(auc.right.child)){
      auc.right.child <- 0
    }
    ...
    if(furthurStop){
      if(auc >= cutoff){
        good.split <- TRUE
      } else if(auc.left.child >= cutoff | auc.right.child >= cutoff){
        good.split <- TRUE
      }
    } else{
      if(auc >= cutoff){
  ...
  is.father.of.subtree.to.merge <- function(d, cutoff, furthurStop){
    is.father <- FALSE
    if(is.good.split(d, cutoff=cutoff, furthurStop=furthurStop) == FALSE & !is.leaf(d)) is.father <- TRUE
    
    return(is.father)
  }
  
  search_subtree <- function(d){
    if(!is.father.of.subtree.to.merge(d, cutoff=cutoff, furthurStop=furthurStop)){
      for (i in seq_len(length(d))){
        if(is.leaf(d[[i]])){
          next()
        }
...
#' get leaf cluster and its associated cells
